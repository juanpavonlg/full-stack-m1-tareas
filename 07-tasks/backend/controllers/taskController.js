...
exports.updateTask = async (req, res) => {
  ...
  if (task.status === "completada") {
    ...
    if (task.status === "pendiente") {
      return res.status(400).json({ message: "Una tarea 'pendiente' sólo puede pasar a 'en progreso'" });
    } else if (task.status === "en progreso") {
      return res.status(400).json({ message: "Una tarea 'en progreso' sólo puede pasar a 'completada'" });
    }
  }
